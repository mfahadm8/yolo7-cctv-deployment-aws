...
def match_ends(data, left_id, right_id, dist_threshold=0.2, frame_threshold=35):
    """
    ...
    print(f'IDs: {left_id} and {right_id} Frame,X,Y distance {fdist} frames,{xdist},{ydist}, total distance {dist}')
...
def join_contiguous(data, ids):
    ...
    for id_1 in ids:
        for id_2 in ids:
            ...
            if match_ends(data, id_1, id_2):
                data.loc[data['object_id']==id_1,'object_id']=id_2

    return data

def renumber_objects(data):
    #order = data.groupby(by='object_id')['Frame'].idxmin().index
    for i, id in enumerate(data['object_id'].unique()):
        data.loc[data['object_id']==id, 'object_id'] = i
    return data
